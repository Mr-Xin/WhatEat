...
@interface WEAppServiceManager ()

@property (nonatomic, strong) Class<UIApplicationDelegate> appDelegateClass;
@property (nonatomic, strong) NSMutableDictionary<NSString*, id<WEAppService>> *servicesMap;

@end

// 由子类实现服务代理方法，父类负责把这些实现添加到 ApplicationDelegate 中
// 这些都需要在 ApplicationDelegate 实例创建之前执行
@implementation WEAppServiceManager

+ (instancetype)sharedManager {
    static WEAppServiceManager *_sharedManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedManager = [WEAppServiceManager new];
    });
...
- (void)registerService:(id<WEAppService>)service {
    ...
    id<WEAppService> pre = self.servicesMap[[service serviceName]];
    if (pre) {
...
- (BOOL)proxyCanResponseToSelector:(SEL)aSelector {
    ...
    [self.servicesMap enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, id<WEAppService> _Nonnull obj, BOOL * _Nonnull stop) {
